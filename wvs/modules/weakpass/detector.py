...
COMMON_CREDENTIALS = [
    ...
    ("admin", "admin123"), ("admin", "root"), ("admin", "1234"),
    ("admin", "12345"), ("admin", "123"), ("admin", "pass"),
    ("root", "root"), ("root", "admin"), ("root", "123456"),
    ("root", "toor"), ("root", "password"),
...
# 常见登录路径
LOGIN_PATHS = [
    "/login", "/admin", "/admin/login", "/admin/login.php",
    "/wp-login.php", "/administrator", "/manager",
    "/user/login", "/api/login", "/auth/login",
    "/admin/admin.php", "/login.jsp", "/login.aspx",
...
class WeakPasswordDetector(DetectionModule):
    ...
    async def _try_login_form(self, login_url: str) -> Optional[Vulnerability]:
        ...
        for username, password in COMMON_CREDENTIALS[:10]:
            try:
                resp = await self._send_request("POST", login_url, {
                    ...
                    if status == 200 and any(m in body for m in success_markers):
                        if not any(m in body for m in fail_markers):
                            return self._create_vuln(
                                url=login_url, param=None, param_type="body",
                                method="POST", payload=f"{username}:{password}",
    ...
    async def _try_phpmyadmin(self, url: str) -> Optional[Vulnerability]:
        ...
        for username, password in COMMON_CREDENTIALS[:15]:
            try:
                resp = await self._send_request("POST", url, {
                    "pma_username": username, "pma_password": password,
                    "server": "1", "lang": "en",
                })
                if resp:
                    body = resp.get("text", "").lower()
                    if "phpmyadmin" in body and ("navigation" in body or "server" in body):
                        if "cannot" not in body and "denied" not in body:
                            return self._create_vuln(
                                url=url, param=None, param_type="body",
                                method="POST", payload=f"{username}:{password}",
                                vuln_type="weak_password",
                                severity=Severity.CRITICAL,
                                confidence=Confidence.HIGH,
            ...
            try:
                ...
                if resp and resp.get("status_code") == 200:
                    ...
                    if "tomcat" in body and ("manager" in body or "applications" in body):
                        return self._create_vuln(
                            url=url, param=None, param_type="header",
                            method="GET", payload=f"{username}:{password}",
                            vuln_type="weak_password",
                            severity=Severity.CRITICAL,
                            confidence=Confidence.HIGH,
