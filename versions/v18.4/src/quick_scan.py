...
async def main():
    ...
    print("[*] Starting scan on http://192.168.18.131")
    result = await scanner.scan('http://192.168.18.131', modules=['sqli', 'xss', 'cmdi', 'lfi', 'nuclei'])
    print()
    print(f"URLs: {len(result.urls)}  Forms: {len(result.forms)}  Duration: {result.duration:.1f}s")
    ...
    for sev in ['critical', 'high', 'medium', 'low', 'info']:
        if by_sev.get(sev):
            ...
            for v in by_sev[sev]:
                print(f"  [{v.source:10s}] {v.type} @ {v.url[:70]}")
    ...
    print(f"\nTotal: {len(result.vulnerabilities)} vulns saved to scan_result.json")
